...
4. v0.4 (in progress): recovery inspection, approval-bound hash-safe rollback, upgrade, and fault injection. Recovery inspection and rollback, `recover list` retained-run discovery with pre-plan blocking on recovery-required runs, an OS advisory project lock, execution of the CREATE plan's declared validations before finalization, and post-publication and pre-validation test faults are implemented; upgrade, `recover discard`, finalized-state cleanup, and the wider fault matrix remain disabled.
