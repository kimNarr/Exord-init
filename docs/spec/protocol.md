...
Adapters create a schema-valid SetupIntent. They cannot supply operations, risk counts, or a trusted hash. The engine inspects the target and exclusively creates the Setup Plan, canonical `spec_sha256`, and Result. An ADOPT plan may include `adopt_analysis` with bounded inventory, Git, Task, known-guidance, and conflict facts; an `UPGRADE` plan (`plan --upgrade`, no intent) includes `upgrade_analysis` with the manifest settings and a per-managed-file assessment. Both stay inside the hashed plan spec.
...
Stdout contains exactly one Result JSON object. Redacted diagnostics use stderr. Exit codes are stable at the category level; Result `code` carries the specific reason. Every `plan` reports `changed: false`; a successful CREATE apply reports `changed: true`. ADOPT and `plan --upgrade` do not produce an applicable run or approval request in the current prototype. `plan --upgrade` requires a valid `.exord/manifest.json`; a missing or foreign manifest is `UNSUPPORTED_TARGET`, and a `schema_version` the engine does not understand is `VERSION_MISMATCH` (every write stops). Recovery inspection reports `changed: false`; recovery rollback requires an Approval bound to `RECOVER_ROLLBACK` and reports whether target or journal state changed. A recovery failure Result carries no recovery analysis; its next action is to re-run `recover inspect` for the current disposition.
...
`recover discard` permanently removes one settled run bundle. Settled means `FAILED`, `FINALIZED`, or an `ACTIVE` run still at stage `PLANNED`/`APPROVED`; `RECOVERY_REQUIRED` and interrupted mid-apply runs are refused (`code: "DISCARD_NOT_ALLOWED"`). It requires an Approval whose `approved_action` is `RECOVER_DISCARD`, bound to the same `run_id`, `plan_id`, `spec_sha256`, and target identity, verifies the stored plan the way rollback does, refuses a bundle path that is not a plain directory, and reports `changed` to reflect whether the bundle was actually removed. A partial removal is `code: "DISCARD_INCOMPLETE"`, never a success. The three recovery approval actions (`RECOVER_ROLLBACK`, `RECOVER_DISCARD`) and `APPLY_CREATE` never substitute for one another.

`plan --upgrade` classifies each managed file into one `upgrade_action` (`UP_TO_DATE`, `TEMPLATE_UPDATE_AVAILABLE`, `PROPOSE_ONLY_UPDATE`, `REVIEW_LOCAL_EDIT`, `RECREATE_MISSING`, `NEW_MANAGED_FILE`, `ADOPT_EXISTING_FILE`, `ORPHAN_MANAGED_FILE`, `BLOCKED`) from its recorded baseline, its `hash_mode` (only `TEXT_UTF8_LF_V1` is understood; anything else is `BLOCKED`), its `update_policy`, and the current template version. The plan-level `disposition` is the worst of `UP_TO_DATE`, `UPGRADE_AVAILABLE`, `REVIEW_REQUIRED`, `BLOCKED`. A recorded generator version newer than the engine's is `direction: "DOWNGRADE"` and forces `BLOCKED`; an unparseable one is `direction: "UNKNOWN"`. The analysis is read-only and reuses the deterministic `spec_sha256`.
