...
빌드된 실행 파일을 `EXORD_INIT_BIN`에 지정하면 실제 `plan -> approval -> apply` schema 통합 테스트도 실행합니다.

```sh
EXORD_INIT_BIN=./bin/exord-init python -m unittest discover -s tests -p '*_test.py'
```

현재 Windows amd64에서 native 실행을 검증했습니다. Linux amd64, macOS arm64와 Windows arm64 교차 컴파일은 성공했지만 native platform matrix와 release artifact 검증은 후속 작업입니다.
